...
public class HiveDB {
	...
	public static String url = Constants.HIVE_URL;
	public static String user = Constants.HIVE_USER; // 主机的用户名
	public static String password = Constants.HIVE_PASSWORD; // 远程主机密码
	...
	public static void main(String[] args) {
		...
		try {
			...
			/*
			 * // 第三步:查看创建的表 showTables(stmt,tableName);
			 * 
			 * // 执行describe table操作 describeTables(stmt,tableName);
			 * 
			 * // 执行load data into table操作 loadData(stmt,tableName);
			 * 
			 * // 执行 select * query 操作 //selectData(stmt,tableName);
			 * 
			 * // 执行 regular hive query 统计操作 countData(stmt,tableName);
			 */
	...
	public static void loadData(Statement stmt, String tableName) throws SQLException {
		...
		sql = "load data localinpath '" + filepath + "' into table " + tableName;
		log.info("Running:" + sql);
		stmt.execute(sql);
	}
